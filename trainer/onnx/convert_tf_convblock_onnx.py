...
from trainer.models.mf import MultiFrameModelConvOnly
import tf2onnx
...
if __name__ == "__main__":
    ...
    model = MultiFrameModelConvOnly(cfg=cfg)
    model.load_weights(cfg.model.weights)
    
    # create inference for model compilation
    input_img = tf.convert_to_tensor(np.zeros((cfg.training.batch_size, cfg.data.res, cfg.data.res, 1), dtype=np.float32))
    _ = model(input_img)

    # save model with graph as pb
    tf.saved_model.save(model, "mfm_convonly")

    # use this command line to convert to onnx:
    # python3 -m tf2onnx.convert --saved-model mfm_convonly --output "model_convonly.onnx"
