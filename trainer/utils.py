...
import cv2
from trainer.models import DeepNetModel, MultiFrameModel
...
class ConfigManager(object):
    ...
    def __init__(self, json_name, retrain=False):
        ...
        if self.mode == 'us2multimidi' or self.mode == 'us2multikey' or self.mode == 'us2conf2multimidi' or self.mode == 'us2conf2multikey':
            self.use_dev = True
        else:
            self.use_dev = False
        ...
        if self.mode == 'us2conf' or self.mode == 'us2conf2multimidi' or self.mode == 'us2conf2multikey':
            self.use_conf = True
        else:
            self.use_conf = False

        # random seed, to make all experiments equal
        if 'RANDOM_SEED' in cfg_dict:
...
class ConfigDataManager(object):
    ...
    def __init__(self, cfg_dict, retrain):
        ...
        # sequence - set if to ask for a bunch of labels (device/hand) in one sample
        ...
        # set number of color channels for each image
        ...
        elif len(self.train_files) > 0 or self.share_train > 0:
            self.single_test = False
        else:
            self.single_test = True
...
def initiate_model(cfg):
    ...
    if cfg.model.type == 'mfm':
        return MultiFrameModel(cfg=cfg)
    else: # cfg.model.type == 'deepnet':
        return DeepNetModel(cfg=cfg)

###################################################
#          Utilities For Testing Methods          #
###################################################

# compute loss stats for configuration prediction
def compute_conf_metrics(conf_y, preds, timestamps, ike, fke, ide, fde):
...
def store_raw_predictions_and_images(data_dir, images, labels, predictions):

    # prepare output folder
    dir_time = datetime.now().strftime("%d-%m-%Y_%H:%M:%S")
    dir_name = os.path.join(data_dir, f'raw_exp_data_{dir_time}')
    ...
    for i in range(len(images)):
        cv2.imwrite(os.path.join(imgs_dir_name, f'{i}.png'), cv2.resize(images[i], (112, 112)))
...
def create_confs_graph(preds_np, labels_np, labels_names):
    ...
    # arrange rows and columns for joints graphs
    n_rows, n_cols = 5, 4
